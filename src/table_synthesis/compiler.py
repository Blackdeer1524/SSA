...
def generate_nonterm_node(name: str, rules: list[list[str]]) -> str:
    ...
    for rule in rules:
        ...
        s = "tuple[{}]".format(", ".join(map(lambda x: f"\"{generate_node_name(x)}\"", rule))) if len(rule) > 1 else f"\"{generate_node_name(rule[0])}\""
        types.append(s)

    node_name = f"{name}Node"
    value_type = 'Optional[Union[{}]]'.format(",".join(types)) if len(types) > 1 else f"Optional[{types[0]}]"
